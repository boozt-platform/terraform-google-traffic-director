...
| <a name="input_create_forwarding_rule"></a> [create\_forwarding\_rule](#input\_create\_forwarding\_rule) | Whether to create the forwarding rule and TCP proxy. Set to false if forwarding rule is not required. | `bool` | `true` | no |
| <a name="input_ip_address"></a> [ip\_address](#input\_ip\_address) | The IP address for the forwarding rule. Use '0.0.0.0' for the default mesh-wide listener, or a different address (e.g., '10.0.0.0') to avoid port conflicts when multiple backend services share the same port. | `string` | `"0.0.0.0"` | no |
